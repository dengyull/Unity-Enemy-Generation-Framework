...
public class IntensityManager_dep : MonoBehaviour {
    ...
    public void EnemyPropertyIncrease() {
        foreach (GEGCharacter character in PackedData.characters) {
    ...
    void EnemyNumberUpdate(int indexs, int number) {
        int t = 0;
        for (int i = 0; i < PackedData.characters.Count; i++) {
            if (PackedData.characters[i].type == CharacterType.Enemy) {
                if (t == indexs) {
                    PackedData.characters[i].numNextWave = number;
                }
                t++;
            }
        }
